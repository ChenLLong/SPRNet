...
def main():
        ...
        if ds == 'ic15':
            ...
            cfg.data.train.append(
                dict(
                    ...
                    pipeline=train_pipeline, regen_ann=True, num_width_line = cfg.model.mask_head.num_width_line, BBox_type=PolyBBox_divide),
            )
            ...
            cfg.data.train.append(
                dict(
                    type='CTW1500Dataset',
                    ann_file=data_root + 'ctw1500/train/gts/train_gts.json',
    ...
    for i, train_ds in enumerate(cfg.data.train):
        ...
        train_ds['pipeline'] = [
            ...
            resize,
            dict(type='TextRandomFlip', flip_ratio=0.5),
            # dict(type='TextMinIoURandomCrop', crop_size=800),
            dict(type='Normalize', **cfg.img_norm_cfg),
            dict(type='Pad', size_divisor=32),
            dict(type='TextDefaultFormatBundle', recognition_predition_type=args.Prediction),
    ...
    if args.mode == 'pretrain' and args.datasets != 'synthtext':
        cfg.total_epochs=80
        cfg.lr_config['step']=[50,70]
        cfg.optimizer = dict(type='SGD', lr=0.01, momentum=0.9, weight_decay=0.0001)
        cfg.exchang_anchor_t_config = dict(exchange_t_epoch=10)
        cfg.use_poly_loss_epoch_config = dict(use_poly_loss_epoch=20)
    # if args.mode == 'pretrain' and args.datasets != 'synthtext':
    #     cfg.checkpoint_config = dict(interval=10)
    #     cfg.total_epochs = 600
    #     cfg.lr_config['step'] = [400, 550]
    #     cfg.optimizer = dict(type='SGD', lr=0.01, momentum=0.9, weight_decay=0.0001)
    #     cfg.exchang_anchor_t_config = dict(exchange_t_epoch=50)
    #     cfg.use_poly_loss_epoch_config = dict(use_poly_loss_epoch=1000)
    if args.mode == 'pretrain' and args.datasets == 'synthtext':
        cfg.total_epochs=2
        cfg.lr_config = dict(
            by_epoch=False,
        ...
        cfg.exchang_anchor_t_config = dict(exchange_t_epoch=10)
        cfg.use_poly_loss_epoch_config = dict(use_poly_loss_epoch=100)
        ...
        # cfg.model.mask_head.num_control_line = 5
        cfg.total_epochs=70
        ...
        # cfg.model.mask_head.num_control_line = 7
        cfg.total_epochs=70
        cfg.lr_config['step']=[40,60]
        cfg.optimizer = dict(type='SGD', lr=0.01, momentum=0.9, weight_decay=0.0001)
        cfg.exchang_anchor_t_config = dict(exchange_t_epoch=10)
